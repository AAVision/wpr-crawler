...
class WorkplaceRelationsSpider(scrapy.Spider):
    ...
    def __init__(self, start_date=None, end_date=None, body_id=None,
                 body_name=None, partition_date=None, **kwargs):
        ...
        yield scrapy.Request(
            ...
            meta={
                ...
                'playwright_page_methods': [
                    PageMethod('wait_for_selector', 'li.each-item, .results-count', timeout=30000),
                ],
            },
    ...
    def parse_search_results(self, response: HtmlResponse):
        ...
        results_text = response.xpath(
            '//text()[contains(., "Shows")]'
        ).re_first(r'Shows\s+\d+\s+to\s+\d+\s+of\s+([\d,]+)\s+results')
        if results_text:
            total = int(results_text.replace(',', ''))
            if page_num == 1:
                self._log_structured('search_results_count', {
                    'total_results': total,
                    'body': self.body_name,
                    'partition_date': self.partition_date,
                })
        ...
        if not result_items:
            self._log_structured('no_results_on_page', {
        ...
        for item in result_items:
            try:
                ...
                self._log_structured('item_parse_error', {
                    'page_number': page_num,
                    'error': str(e),
                })

        self._log_structured('page_parsed', {
            'page_number': page_num,
    ...
    def _extract_identifier(self, response: HtmlResponse) -> str:
        ...
        adj_match = re.search(r'(ADJ-\d+|DET-\d+|DEE-\d+|EDA-\d+|EET-\d+|'
    ...
    def _extract_date(self, response: HtmlResponse) -> str:
        ...
        dated_match = response.xpath(
            '//text()[contains(., "Dated:")]'
        ).re_first(r'Dated:\s*(\d{2}/\d{2}/\d{4})')
        if dated_match:
            return dated_match
    ...
    IGNORED_DOC_PATTERNS = [
        'cookie', 'privacy', 'terms', 'footer', 'policy',
        'accessibility', 'disclaimer', 'sitemap', 'guide',
        'publications_forms', 'information_guide',
    ]
    ...
    def handle_error(self, failure):
        ...
        self._log_structured('request_failed', {
            'url': url,
    ...
    def handle_download_error(self, failure):
        ...
        if item and isinstance(item, DecisionItem):
            ...
            if source_url:
                self._log_structured('download_fallback_html', {
                    'identifier': identifier,
    ...
    def closed(self, reason):
        ...
        self._log_structured('run_summary', {
            'body': self.body_name,
