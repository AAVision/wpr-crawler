...
class HTMLCleaner:
    def __init__(self):
        self.navigation_selectors = [
            ...
            '.social-share', '.related-links', '.comments',
    ...
    def _find_main_content(self, soup):
        content_selectors = [
            ...
            '.decision-content', '.document-content', '#content',
    ...
    def _clean_empty_tags(self, element):
        for tag in element.find_all():
            if not tag.get_text(strip=True) and not tag.find_all():
                tag.decompose()
            elif tag.name in ['span', 'div'] and len(tag.get_text(strip=True)) < 10 and not tag.find_all():
                tag.unwrap()